...
sequenceDiagram
    ...
    FB->>DB: Walidacja studenta, sprawdź uprawnienia (semestr 6/7)
    DB-->>FB: OK / brak uprawnień

    alt Brak uprawnień
        FB-->>FE: 403 + komunikat błędu
        FE-->>S: Wyświetl błąd walidacji
    else Student uprawniony
        FB->>DB: INSERT praktyka {status: "Draft"}
        DB-->>FB: praktyka_id
        ...
        FB->>DB: SELECT praktyka WHERE id=?
        DB-->>FB: dane praktyki
        ...
        alt UOPZ odrzuca (niezgodność z kierunkiem)
            ...
            alt ZOPZ odrzuca harmonogram
                ...
                FB->>DB: UPDATE harmonogram {podpis_ZOPZ: true}
                ...
                FB->>DB: UPDATE harmonogram {podpis_Student: true}
                ...
                FB->>DB: UPDATE harmonogram {status: "Approved"}
                FB->>DB: UPDATE praktyka {status: "Approved"}
                DB-->>FB: OK

                FB->>DB: SELECT dane_studenta, zaklad, harmonogram, opiekunowie
                ...
                FB->>DB: INSERT dokumenty {typ: "zal3", "zal2a"}
